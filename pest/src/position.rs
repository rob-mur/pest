...
#[derive(Clone, Copy)]
pub struct Position<'i, T: ParseInput> {
    input: &'i T,
    /// # Safety:
    ///
...
impl<'i, T: ParseInput> Position<'i, T> {
    /// Create a new `Position` without checking invariants. (Checked with `debug_assertions`.)
    ///
    /// # Safety:
    ///
    /// `input[pos..]` must be a valid codepoint boundary (should not panic when indexing thus).
    pub(crate) unsafe fn new_unchecked(input: &str, pos: usize) -> Position<'_, T> {
    ...
    /// ```
    pub fn new(input: &impl ParseInput, pos: usize) -> Option<Position<'_, T>> {
    ...
    /// ```
    #[inline]
    pub fn from_start(input: &'i str) -> Position<'i, T> {
    ...
    /// ```
    #[inline]
    pub fn span(&self, other: &Position<'i, T>) -> span::Span<'i> {
...
impl<'i, T: ParseInput> fmt::Debug for Position<'i, T> {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        f.debug_struct("Position").field("pos", &self.pos).finish()
    }
}

impl<'i, T: ParseInput> PartialEq for Position<'i, T> {
    fn eq(&self, other: &Position<'i, T>) -> bool {
        ptr::eq(self.input, other.input) && self.pos == other.pos
    }
}

impl<'i, T: ParseInput> Eq for Position<'i, T> {}

impl<'i, T: ParseInput> PartialOrd for Position<'i, T> {
    fn partial_cmp(&self, other: &Position<'i, T>) -> Option<Ordering> {
        if ptr::eq(self.input, other.input) {
...
impl<'i, T: ParseInput> Ord for Position<'i, T> {
    fn cmp(&self, other: &Position<'i, T>) -> Ordering {
        self.partial_cmp(other)
            .expect("cannot compare positions from different strs")
    }
}

impl<'i, T: ParseInput> Hash for Position<'i, T> {
